...
def card(title, description, height, body):
    return (
        ...
        f'<rect x="0.5" y="0.5" width="759" height="{height - 1}" rx="12" '
...
def render_stats(snapshot):
    ...
    for index, (label, value) in enumerate(metrics):
        x = 24 + index * 183
        body += [text(x, 92, f"{value:,}", size=34, color="#58a6ff", weight=600), text(x, 120, label)]
    body += [
...
def render_languages(snapshot):
    items = list(snapshot["languages"].items())
    total = sum(value for _, value in items)
    if len(items) > 6:
        items = items[:5] + [("Other", sum(value for _, value in items[5:]))]
    body = [text(24, 35, "Languages in public projects", size=20, weight=600)]
    for index, (language, size) in enumerate(items):
        y = 73 + index * 30
        share = size / total
        body += [
            text(24, y, language),
            f'<rect x="180" y="{y - 12}" width="480" height="12" rx="4" fill="#21262d"/>',
            f'<rect x="180" y="{y - 12}" width="{480 * share:.2f}" height="12" rx="4" fill="{COLORS[index]}"/>',
            text(680, y, f"{share:.1%}"),
        ]
    if not items:
        body.append(text(24, 75, "No language data is available for public original repositories."))
    footer_y = 85 + max(len(items), 1) * 30
    body += [
        text(24, footer_y, "Share of detected source bytes in owned, non-fork public repositories.", size=12, color="#9198a1"),
        text(24, footer_y + 22, f"Not a proficiency ranking · Updated {snapshot['date']} UTC", size=12, color="#9198a1"),
    ]
    description = "; ".join(f"{language}: {size / total:.1%}" for language, size in items)
    return card("Languages in public projects", description or "No language data available", footer_y + 44, body)
